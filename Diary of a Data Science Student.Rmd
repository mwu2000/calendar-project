...
productivitydata <- mycal %>% filter(timewasted != 20000000) %>%
  mutate("studyarea" = case_when(grepl("Bed", summary, ignore.case = TRUE) ~  "Bed",
                                     ...
                                     TRUE ~ "whatever"),
                                     "timewastedproportion" = timewasted/workmin,
...
violinplot4 <- ggplot(noutliers, aes(x = worktype, y = timewastedproportion, fill = worktype)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + #geom_count()
  geom_jitter(width = 0.05)+
  scale_fill_manual(values = pal)+
  labs(title = "Allotted Work Time That Was Wasted by Assignment Type",
...
productivitydata <- productivitydata %>% mutate("timeproductive" = (workmin - timewasted))
...
linegraph1 <- ggplot(productivitydata, aes(x = daysleft)) + 
  geom_smooth(aes(y = workmin), se = FALSE, fullrange = TRUE) +
  scale_x_reverse() +
  geom_ribbon(aes(ymin = 0,ymax = predict(loess(workmin ~ daysleft))),
                 alpha = 0.4, fill = "#F5CDB4")+
  ...
  geom_ribbon(aes(ymin = 0,ymax = predict(loess(timeproductive ~ daysleft))),
                 alpha = 1.0, fill = "#74A089")
linegraph1
```

```{r}
#Our data has too much variation and too little observations. Let's simulate some more results!
distributionworkmin <- ggplot(productivitydata, aes(x = workmin)) + geom_bar()
distributionworkmin
```

```{r}
#faceted by work type
linegraph2 <- ggplot(productivitydata, aes(x = daysleft)) + 
  geom_smooth(aes(y = workmin), se = FALSE, fullrange = TRUE) +
  scale_x_reverse() +
  geom_ribbon(aes(ymin = 0,ymax = predict(loess(workmin ~ daysleft))),
                 alpha = 0.4, fill = "#F5CDB4")+
